...
function App() {
  const { title, embedUrl, views, createdAt, downvotes, upvotes, comments } =
    video;
  ...
  return (
    <div className="App">
      ...
      <div>
        <button onClick={() => setLike((perv) => perv + 1)}>
          {like + " "} 👍
        </button>

        <button onClick={() => setDisLike((perv) => perv + 1)}>
          {disLike + " "} 👎
        </button>
      </div>

      <button
        onClick={() => setToggle(!toggleComments)}
